...
public class BladeEntityRenderer extends EntityRenderer<BladeEntity> {
    ...
    private final BladeEntityModel model;

    public BladeEntityRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new BladeEntityModel(context.bakeLayer(BladeEntityModel.LAYER_LOCATION));
    }
    ...
    public void render(BladeEntity entity, float yaw, float tickDelta, PoseStack poseStack, MultiBufferSource bufferSource, int light) {
        ...
        VertexConsumer vertexConsumer = bufferSource.getBuffer(net.minecraft.client.renderer.RenderType.lines());
        Vec3 startPos = entity.getOwner().getEyePosition(tickDelta);
        Vec3 endPos = entity.position();
        drawSegmentedChain(vertexConsumer, startPos, endPos);
    }

    private void drawSegmentedChain(VertexConsumer vertexConsumer, Vec3 start, Vec3 end) {
        int segments = 10;
        ...
        for (int i = 1; i <= segments; i++) {
            ...
            float waveOffset = Mth.sin(t * Mth.PI) * 0.2f;
            Vec3 currentPos = start.add(segmentVector.scale(i)).add(0, waveOffset, 0);
            vertexConsumer.vertex((float) prevPos.x, (float) prevPos.y, (float) prevPos.z).color(255, 255, 255, 255).endVertex();
            vertexConsumer.vertex((float) currentPos.x, (float) currentPos.y, (float) currentPos.z).color(255, 255, 255, 255).endVertex();
            prevPos = currentPos;
        }
    }
